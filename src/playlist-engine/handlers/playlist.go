...
func (a *App) HandleGeneratePlaylist(w http.ResponseWriter, r *http.Request) {
	...
	uris, err := a.Spotify.SearchTracks(ctx, token, r8.Genres, r8.TrackCount)
	if err != nil {
	...
	playlistID, err := a.Spotify.CreatePlaylist(ctx, token, userID, playlistName, description)
	if err != nil {
		writeProblem(w, http.StatusBadGateway, "Spotify playlist creation failed", err.Error(), corrID, expID)
		return
	}

	if err := a.Spotify.AddTracks(ctx, token, playlistID, uris); err != nil {
